# CLAUDE.md — Reallog Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Reallog Ai
- **Role**: AI-powered Reallog companion
- **URL**: https://reallog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/reallog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/reallog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://reallog-ai.casey-digennaro.workers.dev/health
curl -s https://reallog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=reallog-ai
...
- **vessel.json capabilities**: content-creation, journalism-tools
...
1. Keep the vessel small. ~234 lines is the current size.
